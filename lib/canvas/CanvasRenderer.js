...
var CanvasRenderer = new Class({
    ...
    drawImage: function(image, x, y, width, height) {
        ...
        //if tinting is enabled, and we are tinting the image other than white
        if (this.tinting && (r !== 255 || g !== 255 || b !== 255)) {
            //if the image doesn't want to use a tint cache, just draw it tinted immediately
            //(i'm avoiding property getters since they are brutally slow on Safari)
            if (!image._tintCaching) {
                var useMultiply = image._tintMode !== Image2D.TintMode.COLORIZE;
                drawTintedImage(useMultiply, ctx, src, this.__lastFillStyle, x, y, width, height);
                return;
            }

            //the tint cache hasn't been created yet...
            if (!image.tintCache)
                image.tintCache = new TintCache(image);

            //OK, we can finally grab our tinted image!
     ...
     * in the tint cache.
     *
     * This will respect the tint mode/fuzziness/size/etc properties of
     * Image2D. If image.tintCaching is set to false, this method has no effect.
     *
     * @param {Image2D} image the Image2D to operate on
     * @param {Array} colors is an array of CSS color strings and/or [r, g, b] array values.
     * @param {Boolean} overwrite whether to overwrite 
     */
    generateTints: function(image, colors) {
        if (!image.tintCaching)
            return;

        if (!image.tintCache)
            image.tintCache = new TintCache(image);

        var len = image.tintCache.size;
        var src = image.source;

        for (var i=0; i<colors.length && i<len; i++) {
            var c = colors[i];

            var t = typeof c === "string"
                        ? colorString.getRgb(c)
                        : c;
            ...
            if ((r===255 && g===255 && b===255) || this.contains(r, g, b))
                continue;

            //caches the specified color
            image.tintCache.tinted( r, g, b );
        }
    }
